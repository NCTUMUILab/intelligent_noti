    ...
    def _find_dirs(self, contact_list):
        ...
        for contact_name in contact_list:
            if contact_name in self._name_path_dict:
                self._target_dir_list.append(
                    self._name_path_dict[contact_name])
...
class LineLogParser:
    ...
    def _print_list_and_select_contact(self, file_name, contact_parsed_list):
        ...
        for index, contact_parsed in enumerate(contact_parsed_list):
            print("\t\t[{:2d}] [{}]  {}".format(
                index, "COMPLETE" if contact_parsed[1] else " NOT YET", contact_parsed[0]))
        print("\tFile to parse: {}".format(file_name))
        contact_id_str = input(
            "\tWhich contact is he/she?(enter the contact id, leave a blank if no match): ")
        if contact_id_str:
            try:
    ...
    def _check_date(self, line):
        # 2018/04/23(週一), 2018/03/19（一）, 2018/04/23(Mon)
        ymd = match('^\d{4}\/\d{2}\/\d{2}', line)
        if ymd:
            self.current_date = ymd.group(0)
            return True

        # Thu, 04/23/2018
        mdy = match('^[A-Z][a-z]{2}, \d{2}\/\d{2}\/\d{4}', line)
        if mdy:
    ...
    def _check_new_message(self, line):
        ...
        # 01:22 Kevin Goodbye
        elif self._special_case and match('^\d{2}:\d{2} ', line):
            try:
