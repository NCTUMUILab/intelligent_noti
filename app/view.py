...
def getLocations():
	marks = []
	locations = db.session.query(Result).filter(Result.r_type == 'gps').filter(Result.user == '1674662e9b773a69').order_by(desc(Result.created_at)).limit(10).all()
...
@app.route('/createGpsLabel', methods=['POST'])
def createGpsLabel():
...
@app.route('/deleteGpsLabel', methods=['POST'])
def deleteGpsLabel():
    data = request.form.to_dict(flat=True)
...
@app.route('/listGpsLabel', methods=['GET'])
def listGpsLabel():
