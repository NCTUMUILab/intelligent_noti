...
class LoginForm(FlaskForm):
    email = StringField('電子信箱', validators=[
                        InputRequired(), Email(message="格式不符合"), Length(max=50)])
...
class RegisterForm(FlaskForm):
    email = StringField('電子信箱', validators=[InputRequired(), Email(
...
class FacebookLoginForm(FlaskForm):
    account = StringField('Email or Phone number',
                          validators=[InputRequired()])
    password = PasswordField('Password', validators=[InputRequired()])


class FacebookResultForm(FlaskForm):
    file_facebook = FileField('result_facebook.txt',
...
class ForgotPassword(FlaskForm):
    email = StringField('email', validators=[InputRequired(), Email(
        message="Invalid email format"), check_mail_exist])
    password = PasswordField('new_password', validators=[InputRequired(), Length(
        min=8, max=80), EqualTo('confirm', message='Passowrds must match')])
